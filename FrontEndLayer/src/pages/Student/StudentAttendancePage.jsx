    ...
    <div className="flex flex-col gap-8 max-w-[960px] mx-auto">
      ...
      <div className="flex gap-4">
        <KPICard label="OVERALL ATTENDANCE" borderColor="#4F378A">
          <div className="flex items-end gap-2">
            ...
            <span className={`font-body text-base pb-1 ${overallPercent >= 80 ? "text-primary" : "text-danger"}`}>
      ...
      <div className="flex flex-col gap-4">
        ...
        {courseAttendance.length === 0 ? (
          ...
          <div key={course.code} className="bg-white border border-border-color rounded-xl p-6 shadow-sm">
            <div className="flex justify-between items-center mb-3">
              <div>
                <span className="font-heading font-semibold text-base text-primary">
                  {course.code}
                </span>
                <span className="font-heading text-base text-text-primary ml-2">
                  {course.name}
                </span>
              </div>
              <span className={`font-heading font-bold text-base ${course.percent >= 80 ? "text-primary" : "text-danger"}`}>
                {course.percent}%
              </span>
            </div>
