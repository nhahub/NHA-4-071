    ...
    <div className="flex flex-col gap-8 max-w-[960px] mx-auto">
      ...
      <div className="flex gap-4">
        <KPICard label="SEMESTER GPA" borderColor="#4F378A">
          <span className="font-heading font-normal text-4xl text-primary">{semesterGpa.toFixed(2)}</span>
        </KPICard>
        <KPICard label="CUMULATIVE GPA" borderColor="#63597C">
          <span className="font-heading font-normal text-4xl" style={{ color: "#63597C" }}>{cumulativeGpa.toFixed(2)}</span>
        </KPICard>
        <KPICard label="TOTAL CREDITS" value={totalCredits} borderColor="#CFBCFF" />
      </div>

      <div className="flex gap-4 bg-bg-light p-4 rounded-xl items-end">
      ...
      <div className="bg-white border border-border rounded-xl overflow-hidden">
        <div className="flex px-6 py-4 border-b border-border bg-bg-light">
        ...
        {courses.map((c, i) => (
          <div key={i} className="flex px-6 py-3 items-center border-b border-border">
            <input value={c.name} onChange={(e) => update(i, "name", e.target.value)} placeholder="Course name"
              className="flex-[2] p-2 border border-border rounded-md font-heading text-sm mr-3" />
            <input value={c.credits} onChange={(e) => update(i, "credits", e.target.value)} placeholder="Credits"
              className="flex-1 p-2 border border-border rounded-md font-heading text-sm mr-3" />
            <select value={c.grade} onChange={(e) => update(i, "grade", e.target.value)}
              className="flex-1 p-2 border border-border rounded-md font-heading text-sm mr-3">
              {Object.keys(gradePoints).map((g) => <option key={g} value={g}>{g}</option>)}
            </select>
            <button onClick={() => removeCourse(i)}
              className="flex items-center gap-2 p-2 bg-transparent text-danger cursor-pointer">
              <Trash2 size={18} />
            </button>
          </div>
        ))}
      ...
      <button onClick={addCourse}
        className="flex items-center gap-2 px-4 py-[9px] bg-bg-light text-primary rounded-lg font-body text-base cursor-pointer self-start">
        <Plus size={16} /> Add Course
      </button>

      <div className="bg-white border border-border rounded-xl p-6 flex items-center gap-4">
        <Calculator size={24} color="#4F378A" />
