...
import { User, Mail, BookOpen, Award, Calendar, Edit3, Save, X } from "lucide-react";
  ...
  return (
    <div className="flex flex-col gap-8 max-w-[960px] mx-auto">
      ...
      <div className="flex gap-6">
        <div className="w-[240px] flex flex-col items-center gap-4 p-8 bg-white border border-border-color rounded-xl shadow-sm self-start">
          <div className="w-24 h-24 rounded-full border-2 border-primary bg-[#E6E0E9] flex items-center justify-center">
            <User size={40} color="#4F378A" />
          </div>
          {editing ? (
            <input
              value={form.name}
          ...
          <div className="bg-white border border-border-color rounded-xl p-6 shadow-sm">
            ...
            <div className="flex flex-col gap-3">
              ...
              {editing ? (
                ...
                <p className="font-heading text-base text-text-primary m-0">{profile?.email || user?.email || ""}</p>
              )}
            </div>
          </div>
...
const ProfileField = ({ icon: Icon, label, value }) => (
  <div className="flex items-center gap-3">
    <div className="w-9 h-9 rounded-lg bg-bg-light flex items-center justify-center">
      <Icon size={18} color="#4F378A" />
    </div>
    <div>
      <p className="font-heading text-xs text-text-muted m-0 tracking-wider uppercase">
        {label}
      </p>
      <p className="font-heading text-base text-text-primary m-0">
        {value}
      </p>
    </div>
  </div>
