...
const StudyPlanPage = () => {
  ...
  return (
    <div className="flex flex-col gap-6 md:gap-8 max-w-[960px] mx-auto">
      ...
      <div className="bg-primary rounded-xl p-4 sm:p-8 flex flex-col gap-3 sm:gap-4">
        ...
        <div className="flex gap-3 sm:gap-4 items-center">
          <span className="font-body text-sm sm:text-base text-[#E0D2FF] opacity-80">
          ...
          <div className="flex flex-col gap-3 sm:gap-4">
            {year.semesters.map((sem) => (
              <div key={sem.name} className="bg-white border border-border-color rounded-xl overflow-hidden shadow-sm">
                <div className="px-4 sm:px-6 py-3 bg-bg-light">
                ...
                <div className="p-3 sm:p-4">
                  {sem.courses.map((course) => (
                    <div key={course.code} className="flex items-center gap-2 sm:gap-3 py-2 border-b border-bg-light">
                      {course.completed ? (
                        <CheckCircle size={18} className="text-primary shrink-0 sm:w-5 sm:h-5" />
                      ) : (
                        <Circle size={18} className="text-[#CBC4D2] shrink-0 sm:w-5 sm:h-5" />
                      )}
                      <div className="flex-1 min-w-0">
                        <span className="font-heading font-semibold text-xs sm:text-sm text-text-primary">
                          {course.code}
                        </span>
                        <span className="font-heading text-xs sm:text-sm text-text-secondary ml-1 sm:ml-2">
                          {course.name}
                        </span>
                      </div>
                      <span className="font-heading text-xs sm:text-sm text-text-muted shrink-0">
                        {course.credits} cr
                      </span>
                    </div>
                  ))}
