...
const SemesterRegistrationPage = () => {
  ...
  return (
    <div className="flex flex-col gap-8 max-w-[960px] mx-auto">
      ...
      <div className={`flex flex-col p-6 gap-4 ${registrationStatus === "open" ? "bg-primary/5 border-primary" : "bg-bg-light border-border"} rounded-xl border`}>
      ...
      <div className="flex gap-4 items-center flex-wrap">
        {steps.map((step, i) => (
          <div key={step} className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full ${i < currentStep ? "bg-primary" : i === currentStep ? "bg-primary-light" : "bg-bg-light"} flex items-center justify-center ${i <= currentStep ? "text-white" : "text-text-muted"} font-heading font-semibold text-sm`}>
              {i < currentStep ? <Check size={16} /> : i + 1}
            </div>
            <span className={`font-heading text-sm ${i <= currentStep ? "text-text-primary" : "text-text-muted"}`}>
              {step}
            </span>
            {i < steps.length - 1 && <ChevronRight size={16} color="#CBC4D2" />}
          </div>
        ))}
      </div>
      ...
      <div className="bg-white border border-border rounded-xl p-8 shadow-sm">
        {currentStep === 0 && (
          <div>
            ...
            <p className="font-heading text-text-secondary m-0">
            ...
            <button
              ...
              className="mt-4 px-4 py-[9px] bg-primary text-white border-none rounded-lg font-body text-base cursor-pointer"
            >
          ...
          <div>
            ...
            {enrolledCourses.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                <p className="font-heading text-text-secondary m-0 mb-2">
        ...
        {currentStep === 2 && (
          <div>
            ...
            <p className="font-heading text-text-secondary leading-normal">
        ...
        {currentStep === 3 && (
          <div className="text-center">
            ...
            <h3 className="font-heading font-semibold text-2xl m-0 mb-2 text-text-primary">
        ...
        <div className="flex justify-between mt-8">
          <button
            ...
            className={`px-4 py-2 border border-text-muted rounded-lg bg-transparent ${currentStep === 0 ? "text-border cursor-not-allowed" : "text-primary cursor-pointer"} font-body text-base`}
          >
            Previous
          </button>
          ...
          <button
            ...
            className={`px-4 py-[9px] ${currentStep === 3 ? "bg-bg-light text-text-muted" : "bg-primary text-white"} border-none rounded-lg font-body text-base ${currentStep === 3 || (currentStep === 2 && (!agreed || submitting)) ? "cursor-not-allowed" : "cursor-pointer"}`}
          >
