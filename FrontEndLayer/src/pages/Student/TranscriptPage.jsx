    ...
    <div className="flex flex-col gap-8 max-w-[960px] mx-auto">
      <PageHeader title="Transcript" subtitle="Academic record and semester breakdown">
        <button onClick={() => window.print()}
          className="flex items-center gap-2 px-4 py-2 border border-text-muted rounded-lg bg-transparent text-primary font-body text-base cursor-pointer">
      ...
      <div className="flex flex-col gap-6">
        {semesters.map((sem, si) => (
          <div key={sem.name} className="bg-white border border-border-color rounded-xl overflow-hidden shadow-sm">
            <div className="flex justify-between items-center px-8 py-3 bg-bg-light">
              <span className="font-heading font-bold text-base text-primary">{sem.name}</span>
              <div className="flex items-center gap-6">
                <span className="font-heading font-semibold text-xs tracking-wider uppercase text-text-secondary">
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-border-color">
                    <th className="text-left px-8 py-4 font-heading font-semibold text-xs tracking-wider uppercase text-text-secondary">Code</th>
                    <th className="text-left px-8 py-4 font-heading font-semibold text-xs tracking-wider uppercase text-text-secondary">Course</th>
                    <th className="text-left px-8 py-4 font-heading font-semibold text-xs tracking-wider uppercase text-text-secondary">Credits</th>
                    <th className="text-right px-8 py-4 font-heading font-semibold text-xs tracking-wider uppercase text-text-secondary">Grade</th>
                  </tr>
                </thead>
                <tbody>
                  {(sem.courses || []).map((course, ci) => (
                    <tr key={course.code} className={ci > 0 ? "border-t border-border-color" : ""}>
                      <td className="px-8 py-[18.5px] font-body text-base text-primary">{course.code}</td>
                      <td className="px-8 py-4 font-heading text-base text-text-primary">{course.name}</td>
                      <td className="px-8 py-4 font-heading text-base text-text-primary">{course.credits}</td>
                      <td className="px-8 py-[18.5px] font-heading font-bold text-base text-warning text-right">{course.grade}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
